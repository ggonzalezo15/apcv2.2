-- ============================================================
-- ESQUEMA CORREGIDO PARA HOSTINGER
-- Generado: 2025-08-16 22:52:08
-- Orden de tablas optimizado para evitar errores de FK
-- ============================================================
...
CREATE TABLE `income_lines` (
  ...
  FOREIGN KEY (`job_types_id`) REFERENCES `job_types` (`id`) ON DELETE SET NULL ON UPDATE CASCADE,
...
SELECT 
    ...
    COALESCE(SUM(CASE WHEN `tr`.`type` = 'expense' THEN ABS(`tr`.`amount`) ELSE 0 END), 0) AS `total_expenses`,
    COUNT(`tr`.`id`) AS `total_transactions`
FROM `bank_accounts` `ba` 
LEFT JOIN `transactions` `tr` ON (`ba`.`id` = `tr`.`bank_account_id`)
GROUP BY `ba`.`id`, `ba`.`name`, `ba`.`bank_name`, `ba`.`account_number`, `ba`.`balance`;
...
SELECT 
    ...
    COALESCE(SUM(CASE WHEN `i`.`status` = 'overpaid' THEN `i`.`total_amount` ELSE 0 END), 0) AS `overpaid_income`
FROM `incomes` `i` 
...
-- Usuario administrador por defecto (CAMBIAR CONTRASEÑA DESPUÉS DEL PRIMER LOGIN)
